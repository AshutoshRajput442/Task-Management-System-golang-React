...
const NavBar = () => {
  ...
  const handleLogout = () => {
    ...
    navigate("/"); // Navigate to login or landing page
  };
    ...
    <nav className="w-64 bg-[#1a1a28] text-gray-400 flex flex-col p-6">
      ...
      <div className="flex flex-col items-center mb-8">
        ...
        <p className="text-white font-semibold">
          Welcom, {name.charAt(0).toUpperCase() + name.slice(1).toLowerCase()}
        </p>
      </div>
      {/* Navigation links */}
          ...
          <li key={index}>
            {item.isLogout ? (
              <button
                ...
                className="flex w-full items-center gap-3 px-3 py-2 rounded-md transition-colors duration-200 hover:bg-gray-700 text-left"
              >
